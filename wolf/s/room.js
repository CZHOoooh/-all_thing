...
function RoomInfo(room, username) {
    $.get("room", { room: room, user: username }, function (data, status) {
        //Get information
...
function ButtonFunction() {
    $("#reissue").click(function () {
        location.href = "reissue?room=" + GetUrlParam('room') + "&playerNo=" + player;
    });
    ...
    $("#nightInfo").click(function () {
        ...
        $.get("play", { acquire3: true, item: "death1", item2: "death2", item3: "death3", room: GetUrlParam('room') }, function (data, status) {
            var deadInfo = "";
            var d = data.split(" ");
...
function StartVoice(){
    $("#startGame").click(function () {
        $.get("play", { room: GetUrlParam('room'), start: true }, function (data, status) {
...
function PrepareChooseBoard(selfRole) {
    if (selfRole == "w-devil") {
        ...
        //女巫
        $.get("play", { acquire3: true, item: "death1", item2: "g-witch", item3: "death2", room: GetUrlParam('room') }, function (data, status) {
            d = data.split(" ");
            $("#chooseTitle").html("女巫");
...
function CheckPredecessorStatus(preRole, callback) {
    $.get("play", { role: preRole, verify: true, room: GetUrlParam('room') }, callback);
}
